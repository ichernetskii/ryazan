...
if(empty($errors)) {
    ...
    $message  = "Здравствуйте,<br/> Подобранные для вас спецпредложения во вложении к письму. ";

    $files = array(
        __DIR__ . '/img/routes-4-info.jpg',
    );

    if (send_mail_attachments($data->email, $message, "Спецпредложения", $files, true) /* отправим письмо */ ) {
